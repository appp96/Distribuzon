...
type ProveedoresFormValues = z.infer<typeof formSchema>;
...
export const ProveedoresForm: React.FC<ProveedoresFormProps> = ({
  ...
  const title = initialData ? "Editar proveedor" : "Alta de proveedor";
  const description = initialData ? "Editar a proveedores" : "Agregar proveedor nuevo";
  ...
  const form = useForm<ProveedoresFormValues>({
    resolver: zodResolver(formSchema),
    defaultValues,
  });

  const onSubmit = async (data: ProveedoresFormValues) => {
    try {
      setLoading(true);
      if (initialData) {
  ...
  const onDelete = async () => {
    try {
      ...
      router.refresh();
      router.push(`/${params.storeId}/proveedores`);
    } catch (error: any) {
    } finally {
      setLoading(false);
      setOpen(false);
