...
type ComprasFormValues = z.infer<typeof formSchema>;
...
export const ComprasForm: React.FC<ComprasFormProps> = ({
  ...
  const title = initialData ? "Editar artículo" : "Alta de compra";
  const description = initialData ? "Editar a compra" : "Agregar compra";
  const toastMessage = initialData ? "Compra actualizada" : "Compra creada";
  ...
  const form = useForm<ComprasFormValues>({
    resolver: zodResolver(formSchema),
    defaultValues,
  });

  const onSubmit = async (data: ComprasFormValues) => {
    try {
      setLoading(true);
      if (initialData) {
  ...
  const onDelete = async () => {
    try {
      ...
      router.refresh();
      router.push(`/${params.storeId}/compras`);
    } catch (error: any) {
    } finally {
      setLoading(false);
      setOpen(false);
